...
bot.on('guildMemberAdd', async(member) => { // this event gets triggered when a new member joins the server!
    ...
    const Channel = member.guild.channels.cache.get('856996227040542780') //insert channel id that you want to send to
    //making embed
...
bot.on('guildMemberRemove', async(member) => { // this event gets triggered when a new member leaves the server!
    // Firstly we need to define a channel
    // either using .get or .find, in this case im going to use .get()
    const Channel = member.guild.channels.cache.get('856996293335449601') //insert channel id that you want to send to
    //making embed
    const embed = new MessageEmbed()
        .setColor('RED')
...
bot.on('guildMemberAdd', async member => {
    ...
    let welcome = await jimp.read('https://cdn.discordapp.com/attachments/858426091764842496/858459268951965726/Background.png')
